...
use std::str::FromStr;
use cosmwasm_std::{Addr, Attribute, BankMsg, coin, Decimal, Deps, Event, Response, StdError, to_binary, Uint128, WasmMsg};
...
impl ResponseHandler {
    pub fn init_response() -> Self {
        return ResponseHandler {
            response: Response::new().add_attribute(
                "action",
                "Instantiate NFT Marketplace contract"
            ) }
    }
    ...
    pub fn add_nft_collection(nft_collection_address: &str) -> Self {
        ...
        response = response
            ...
            .add_attribute("collection address", nft_collection_address.clone().to_string());
        return ResponseHandler {
            response
        }
    }
    ...
    pub fn claim_marketplace_fees(owner: String, vec_of_fees: Vec<DenominationValue>) -> Self {
        ...
        let vec_of_messages: Vec<BankMsg> = vec_of_fees
            ...
            amount: vec![coin(denom_value.value.clone().u128(), denom_value.denom.clone())],
        } ).collect();
        response = response
            .add_event(Event::new("Claim marketplace fees"))
            .add_messages(vec_of_messages.clone());
        ResponseHandler { response: response.clone() }
    }
    ...
    pub fn execute_update_sale(
        ...
        return ResponseHandler { response: response.clone() }
    }
    ...
    pub fn execute_accept_offer(
        ...
        let mut response = Response::new().add_event(Event::new("Accept Offer"))
            .add_attribute("Accepter", accepter.to_string())
            .add_attribute("Offerer", offerer.to_string());
        if !cancel_sale.is_none() {
            response = response.clone().add_message(cancel_sale.unwrap());
        }
    ...
    pub fn register_nft_sale_response(
        ...
        create_profile_msg: Option<WasmMsg>) -> Self {
        let mut response = Response::new();
        if !create_profile_msg.is_none() {
            response = response.add_message(create_profile_msg.unwrap());
        }
        response = response
            .add_event(Event::new("Register NFT For Sale")
            .add_attribute("Seller", nft_for_sale.seller.to_string())
            .add_attribute("Token", nft_for_sale.token_id.to_string())
            .add_attribute("Collection", nft_for_sale.nft_collection_address.to_string())
            .add_attribute("Price", nft_for_sale.sale_price_value.to_string())
            .add_attribute("Denom", nft_for_sale.sale_price_denom.to_string()));
    ...
    pub fn expired_nft_sale_response(buyer: Buyer) -> Self {
        let response = Response::new()
            .add_message(BankMsg::Send {
                to_address: buyer.sender.clone().to_string(),
                amount: vec![coin(buyer.amount.clone().u128(), buyer.denom.clone())],
            })
            .add_event(Event::new("Purchase Cancelled")
                .add_attribute("Sale status", "Expired")
                .add_attribute("Fund value returned", buyer.amount.clone().to_string())
                .add_attribute("Fund denom returned", buyer.denom.clone().to_string())
                .add_attribute("Fund returned to buyer", buyer.sender.clone().to_string()));
    ...
    pub fn cancel_nft_sale_response(nft_for_sale_info: NftSale) -> Self {
        let response = Response::new()
            .add_event(Event::new("Sale Cancelled")
                .add_attribute("Sale status", "Cancelled")
                .add_attribute("Token ID", nft_for_sale_info.token_id.to_string())
                .add_attribute("Collection", nft_for_sale_info.nft_collection_address.to_string())
            );
        Self { response }
    }
    ...
    pub fn execute_succes_nft_sale_response(
        ...
        for vip_lvl in reward_system.vip_perks {
            if Some(vip_lvl.vip_level) == seller_profile.vip_level.clone() {
                discount_from_vip = discount_from_vip.checked_sub(vip_lvl.marketplace_fees_discount)?;
            }
        }

        // For each sale and buy -> send reward tokens
        let mut vec_cw20_transfer: Vec<WasmMsg> = Vec::with_capacity(2 as usize);
        for addr in vec![buyer.sender.clone(), seller_profile.address.clone()].into_iter() {
            let reward_tokens_to_dist: Uint128 = realised_sale_value_usdc
                .checked_div(reward_system.reward_token_per_1usdc_volume)?
                .checked_mul(Uint128::new(1_000_000u128))?;
        ...
        let mut response = Response::new();
        let total_sale_price_value = nft_for_sale_info.sale_price_value.clone();
        let marketplace_revenues = NftSale::compute_marketplace_fees(marketplace_fees_pct.checked_mul(discount_from_vip)?, nft_for_sale_info.sale_price_value);
        ...
        let messages_for_royalties: Vec<BankMsg> = royalties
            ...
            .map(|royalty| BankMsg::Send {
                ...
                amount: vec![coin(royalty.royalty_amount.clone().u128(), nft_for_sale_info.sale_price_denom.clone())],
            })
            .collect();
        ...
        response = response
            .add_event(Event::new("NFT Sold")
                ...
                .add_attribute("Sold in denom", nft_for_sale_info.sale_price_denom.to_string())
                .add_attribute("Marketplace fees", marketplace_revenues.to_string())
                .add_attributes(attributes_for_royalties))
            .add_message(BankMsg::Send {
                to_address: nft_for_sale_info.seller.to_string(),
                amount: vec![coin(
                    total_sale_price_value.u128() -
                        marketplace_revenues.clone().u128() -
                        royalties.iter().map(|r| r.royalty_amount.clone()).sum::<Uint128>().u128(),
                    buyer.denom.clone())],
            })
            .add_message(exec_cw721_transfer)
            .add_messages(vec_cw20_transfer);
        if messages_for_royalties.len() > 0 {
            response = response.add_messages(messages_for_royalties);
        }
        Ok(ResponseHandler { response })
    }

    pub fn execute_accept_offer_but_expired(
        cancel_msg: WasmMsg,
        offer: NftOffer,
    ) -> Result<Self, StdError> {
        let mut response = Response::new();
        response = response
            .add_event(Event::new("Accepted Expired Offer")
                ...
                .add_attribute("Offer in denom", offer.offer_price_denom.clone().to_string()))
            .add_message(BankMsg::Send {
                to_address: offer.offerer_address.clone().to_string(),
                amount: vec![coin(
                    offer.offer_price_value.clone().u128(),
                    offer.offer_price_denom.clone())],
            })
    ...
    pub fn level_up_profile(
        ...
        let response = Response::new().add_event(Event::new("Level Up Profile"))
    ...
    pub fn create_or_update_profile(
        ...
        let response = Response::new().add_event(Event::new("Profile"))
            .add_attribute("Profile Address", profile.address)
            .add_attribute("Profile Picture Collection", if profile.profile_nft_collection.is_none() {
                "None".to_string()
            } else {
                profile.profile_nft_collection.unwrap().to_string()
            })
            .add_attribute("Profile Picture Token ID", if profile.profile_nft_token_id.is_none() {
                "None".to_string()
            } else {
                profile.profile_nft_token_id.unwrap().to_string()
            })
            .add_attribute("Profile Background Collection", if profile.background_nft_collection.is_none() {
                "None".to_string()
            } else {
                profile.background_nft_collection.unwrap().to_string()
            })
            .add_attribute("Profile Picture Token ID", if profile.background_nft_token_id.is_none() {
                "None".to_string()
            } else {
                profile.background_nft_token_id.unwrap().to_string()
            })
            .add_attribute("Profile Description", if profile.description.is_none() {
                "None".to_string()
            } else {
                profile.description.unwrap().to_string()
            })
            .add_attribute("Profile Links", if profile.links.is_none() {
                "Links not updated"
            } else {
                "Links updated"
            })
            .add_attribute("Profile NFT Showcase", if profile.nft_showcase.is_none() {
    ...
    pub fn nft_offer_response(nft_offer: NftOffer, create_profile_msg: Option<WasmMsg>) -> Self {
        ...
        response = response
            .add_event(Event::new("NFT Offer".to_string())
    ...
    pub fn nft_cancel_offer_response(nft_offer: NftOffer) -> Self {
        let response = Response::new()
            .add_event(Event::new("NFT Offer Cancellation")
                .add_attribute("Offerer", nft_offer.offerer_address.clone().to_string())
                .add_attribute("Token ID", nft_offer.token_id.to_string())
                .add_attribute("Collection", nft_offer.nft_collection_address.to_string())
                .add_attribute("Amount Offered", nft_offer.offer_price_value.clone().to_string())
                .add_attribute("Denom offered", nft_offer.offer_price_denom.clone().to_string())
                .add_attribute("Expiration", nft_offer.offer_expiration.to_string()))
            .add_message(BankMsg::Send {
                to_address: nft_offer.offerer_address.clone().to_string(),
                amount: vec![coin(nft_offer.offer_price_value.clone().u128(), nft_offer.offer_price_denom.clone().to_string())],
            });
        Self { response }
    }
}