...
impl AcceptedDenominations {
    pub fn new(
        list_of_denoms: Vec<Denomination>
    ) -> Self {
    ...
    pub fn add_many(
        &mut self,
        denoms: Vec<Denomination>
    ) -> &mut Self {
        for denom in denoms {
            if !self.list_of_denoms.contains(&Denomination::from(denom.clone())) {
                self.add(denom.clone())
            }
        }
        self
    }
    pub fn remove_many(
        ...
        for denom in denoms { self.remove(denom) }
        self
    }
    pub fn add(
        &mut self,
